...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        curr_frame = procedure.env.make_call_frame(procedure.formals, args)
...
def check_formals(formals):
    ...
    while vals != nil:
        if vals in recorded:
            raise SchemeError("Parameters repeated")
        if not isinstance(vals.first, str):
            raise SchemeError("Parameters are not well-formed list of symbols")
        recorded.append(vals.first)
        vals = vals.second
