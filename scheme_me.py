...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        curr_frame = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, curr_frame)
...
def apply_primitive(procedure, args, env):
    ...
    while args is not nil:
        args_in_py.append(args.first)
        args = args.second
    if procedure.use_env:
        args_in_py.append(env)
    try:
        return procedure.fn(*args_in_py)
...
def do_let_form(vals, env):
    ...
    while bindings is not nil:
        names = Pair(bindings[0][0], names)#why not py list?
        vals = Pair(scheme_eval(bindings[0][1], env), vals)
...
def do_if_form(vals, env):
    ...
    if scheme_true(scheme_eval(vals[0], env)):
        return vals[1] #No need eval? cause eval in scheme_eval again
    else:
        return vals[2]
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            ...
            if len(clause.second) > 1:
                return scheme_eval(Pair('begin', clause.second), env) #do begin form?
            elif clause.second is not nil:
...
def do_begin_form(vals, env):
    ...
    check_form(vals, 1)
    "*** YOUR CODE HERE ***"
    for i in range(len(vals) - 1): #use py list?
...
def check_formals(formals):
    ...
    """
    "*** YOUR CODE HERE ***" #could the tra b/c this?
